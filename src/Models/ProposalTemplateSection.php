...
class ProposalTemplateSection extends Model implements Auditable
{
        ...
        switch ($this->section_type) {
            case 'cover_page':
                if (empty($this->content) && !$this->is_dynamic) {
                    $errors[] = 'Cover page requires content or dynamic generation';
                }
                break;
            ...
            case 'acceptance':
                if (empty($this->content)) {
                    $errors[] = ucfirst(str_replace('_', ' ', $this->section_type)) . ' sections require content';
                }
                break;
    ...
    public static function getDefaultContent($sectionType)
    {
        $defaults = [
            'cover_page' => [
                'title' => 'Proposal',
                'content' => '<div class="cover-page"><div class="cover-branding">{{company_logo}}</div><div class="cover-title"><h1>Business Proposal</h1><h2>For {{customer_name}}</h2></div><div class="cover-details"><p>Proposal Number: {{quote_number}}</p><p>Date: {{current_date}}</p><p>Prepared by: {{company_name}}</p></div></div>',
            ],
            'toc' => [
            ...
            'terms_conditions' => [
                ...
                'content' => '<h3>General Terms</h3><p>These terms and conditions form part of any contract for the supply of goods and services by {{company_name}}.</p><h3>Acceptance</h3><p>This proposal is valid for 30 days from the date of issue. Acceptance of this proposal constitutes acceptance of these terms and conditions.</p><h3>Payment</h3><p>Payment terms are as specified in the Payment Terms section.</p><h3>Liability</h3><p>Our liability is limited to the value of the services provided under this agreement.</p>',
            ],
            'review_log' => [
                'title' => 'Review Log',
                'content' => '<table class="review-log"><thead><tr><th>Version</th><th>Date</th><th>Changes</th><th>Reviewed By</th></tr></thead><tbody><tr><td>1.0</td><td>{{current_date}}</td><td>Initial proposal</td><td>{{project_manager}}</td></tr></tbody></table>',
            ],
            'overview' => [
                'title' => 'Overview',
                'content' => '<h1>Executive Summary</h1><p>This proposal outlines our recommended solution for {{customer_name}}.</p><h2>Project Scope</h2><p>The scope of this project includes...</p><h3>Key Benefits</h3><ul><li>Benefit 1</li><li>Benefit 2</li><li>Benefit 3</li></ul><h2>Implementation Timeline</h2><p>The proposed timeline for implementation is...</p>',
            ],
            'acceptance' => [
                'title' => 'Acceptance',
                'content' => '<p>Please review this proposal and provide your acceptance.</p>',
            ],
            'quote_items' => [
                'title' => 'Proposed Solution & Pricing',
                'content' => '<!-- Auto-generated from quote data following original quote structure -->',
            ],
            'payment_terms' => [
                'title' => 'Payment Terms',
                'content' => '<h3>Payment Schedule</h3><p>Payment is due within 30 days of invoice date.</p><h3>Payment Methods</h3><p>We accept payment by bank transfer, credit card, or check.</p><h3>Due Date</h3><p>This proposal expires on {{due_date}}.</p>',
            ],
            'agreement_signature' => [
                'title' => 'Agreement & Signatures',
                'content' => '<div class="signature-section"><h3>Client Acceptance</h3><p>By signing below, the client accepts this proposal and agrees to the terms and conditions outlined herein.</p><div class="signature-block"><div class="signature-line"><p>Client Signature: ___________________________ Date: ___________</p><p>Print Name: ___________________________ Title: ___________</p></div></div><div class="signature-block"><div class="signature-line"><p>{{company_name}} Representative: ___________________________ Date: ___________</p><p>Print Name: ___________________________ Title: ___________</p></div></div></div>',
            ],
        ];
